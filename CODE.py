...
# Конфіг
# =======================
DATA_DIR = Path("MSRCaseStudy/ganttJSON")    # тут повинні лежати methods.json, requirements.json, traces.json
...
class CodeBertEmbedder:
    ...
    def _safe_load(self, model_name: str, local_model_dir: Optional[str]):
        ...
        try:
            ...
            raise RuntimeError(
                "❌ Неможливо завантажити 'microsoft/codebert-base'. Зробіть префетч або вкажіть local_model_dir. "
                f"Деталі: {e3}"
            )
...
def compute_local_metrics(pred_ids: List[int], gold_ids: Set[int]) -> Dict[str, float]:
    ...
    # AP (правильно): сума precision@i на релевантних позиціях / |gold|
...
# =======================
# GUI (адаптивний, із графіками, фікс закриття, старт фулскрін)
# =======================
def launch_gui(requirements, methods, retrieved_map, gold_truth):
    ...
    # --- Вікно ---
    root = tk.Tk()
    root.title("🔎 Traceability GUI (GOLD + графіки метрик)")
    ...
    def show_methods():
        ...
        detailed_txt.insert(tk.END, f"📝 Детальний вивід для {req.requirementname} (Top-{top_k}):\n")
        for i, (mid, score) in enumerate(preds_for_req, 1):
            m = meth_dict.get(mid)
            name = (m.fullmethod.split('.')[-1] if (m and m.fullmethod) else str(mid))
    ...
    # --- глобальні (середні) метрики по всіх вимогах ---
    def show_global_metrics():
        ...
        detailed_txt.insert(tk.END, f"Кількість вимог з gold-трасами: {cnt+1}\n\n")
